 ...
 * developed way more, would od it If I had time.
 */
@SpringComponent
@UIScope
public class InventoryEditorImpl extends VerticalLayout implements KeyNotifier, InventoryEditor {
  ...
  void setupBinders() {
    ...
    binder
        .forField(name)
        .withValidator(
            name -> name.length() > 2, "Name length must be at least two characters long.")
    ...
    binder
        ...
        .withValidator(
            quantity -> (quantity + itemRepository.getTotalQuantity() <= 200),
