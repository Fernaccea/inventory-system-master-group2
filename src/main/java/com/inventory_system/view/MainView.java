...
public class MainView extends VerticalLayout {
  ...
  final Select<String> selectName;

  final Label totalQuantity;

  final Label totalPrice;
  ...
  public MainView(ItemRepository repo, InventoryEditorImpl editor) {
    ...
    this.totalPrice = new Label("Total price: " + repo.getTotalPrice());
    this.addNewBtn = new Button("New item", VaadinIcon.PLUS.create());
    this.selectFilterCategory =
        new Select<>("Show all", "By price", "By categories","By name", "5 last added");
    this.selectCategory = new Select<String>();
    selectCategory.setItems(repo.getAllCategories());

    this.selectName = new Select<String>();
    selectName.setItems(repo.getAllNames());
  ...
  */
  ...
  void buildUI() {
    ...
    grid.setHeight("300px");
    grid.setColumns("id", "name", "price", "category", "quantity", "total_price");
  ...
  void setInitialUIValues() {
    ...
    */
    ...
    String name = repo.findFirstName();
  ...
  // Listen to changes made by the editor, refresh data from backend
    editor.setChangeHandler( () -> {
      ...
      // Update totalQuantity label each time edit happens
      totalPrice.setText("Total price: " + repo.getTotalPrice());
      ...
      // Update the names each time some edit happens
      selectName.setItems(repo.getAllNames());

      // Setting default value again
      selectName.setValue(repo.findFirstName());
    });
  }
  ...
  void setupListeners() {
    ...
    selectName.addValueChangeListener(e -> listItems());
  }
  ...
  void listItems() {
    ...
    } else if (selectFilterCategory.getValue().equals("By name")) {
      grid.setItems(repo.findByNameEquals(selectName.getValue()));
    } else {
      grid.setItems(repo.findAll());
    }
  }
